...
var compiledRecord = require('./middlewares/sourcerecord');
...
function liveReloadSocket(){
  ...
  io.on('connection',function(socket){
    ...
    watcher.add([
      ...
      "/Users/lyt9304/workspace/nproxy-plus/mytest/files"
    ]);
    ...
    watcher
      ...
      .on('change', function (file) {
        ...
        if(/\.js/.test(file)){
          ...
          //rollup & webpack: compiled Source Map
          log.debug("compiledRecord: " + JSON.stringify(compiledRecord.getRecord()));

          files = files.concat(compiledRecord.findInfluencedFiles(file));
          log.debug(JSON.stringify(compiledRecord.findInfluencedFiles(file)));
          log.debug(JSON.stringify(files));

          if(files && files.length){
            for (var i = 0; i < files.length; i++) {
              var obj = files[i];
              log.debug("changed file:"+obj);
              log.debug("changed file:"+path.basename(obj));
              socket.emit("change",{name:path.basename(obj)});
            }
          }
        }
