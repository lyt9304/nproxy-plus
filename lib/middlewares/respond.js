...
var responders = {
  ...
  'babel-webpack' : require('./responders/babel-webpack'),
  stylus : require('./responders/stylus'),
  rollup : require('./responders/rollup')
};
 ...
 * TODO:watchPath--script-bundle.js 也要watch因为会影响rule-file
...
function respond(responderListFilePath, watchPath){
  ...
  var resolveResponderFile = path.resolve(responderListFilePath);
  var resolveWatchPath;
  if(watchPath && watchPath != responderListFilePath){
    resolveWatchPath = path.resolve(watchPath);
  }

  function emptyLoadedCache(){
    log.debug('empty loaded cache');
    
    for(var key in require.cache){
      if(
         key.indexOf(resolveResponderFile) == 0 ||
         resolveWatchPath && key.indexOf(resolveWatchPath) == 0
      ){
        log.debug('delete reuqire cache: ' + key);
  ...
  return function respond(req, res, next){
    ...
    //log.debug('responderList' + JSON.stringify(responderList[0]));
    ...
    for(var i = 0, len = responderList.length; i < len; i++){
      ...
      if(matchFn(url, pattern, req, res)){
        ...
        //只有当baidu script用web的方式替换才需要修正file中的$1
        if(responder === 'web'){
          log.debug('before fix responder: ' + options.file);
          options.file = fixResponder(url, pattern, options.file);
          log.debug('after fix responder: ' + options.file);
        }
...
function _watchRuleFile(file, callback){
  ...
  watcher.on('change', function(path){
    log.debug("chokidar watcher: "+path+"has been changed");
    callback();
  });
};
